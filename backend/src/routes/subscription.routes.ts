...
// CSV Upload Route — using multer now
router.post('/upload-csv', ensureAdminAuthenticated, upload.single('csvFile'), async (req: Request, res: Response): Promise<void> => {
  console.log('📂 Received CSV upload request');
  ...
  if (csvFile.mimetype !== 'text/csv' && !csvFile.originalname.toLowerCase().endsWith('.csv')) {
    console.error('❌ Invalid file type');
    res.status(400).json({ error: 'Only CSV files are allowed' });
    return;
  }
  ...
  try {
    const fileStream = Readable.from(csvFile.buffer);
    const result = await parseSubscribersCsv(fileStream);
